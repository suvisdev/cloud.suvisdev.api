...
> **용도:** `suvisdev/` 에서 ORM 모델·테이블·마이그레이션을 추가·수정할 때 `@suvisdev/_claude/ENTITY_RULE.md` 로 참조한다.  
> 레이어·API 규칙은 루트 [`CLAUDE.md`](../../../CLAUDE.md) §B–I 를 함께 본다.
...
- `suvisdev/apps/mova/app/models/base.py` — `MovaModel` (공통 `id` PK)
- `suvisdev/apps/mova/app/models/chat_model.py` — `chat`
- `suvisdev/apps/mova/app/models/movies_model.py` — `movies`
- `suvisdev/apps/mova/app/models/actors_model.py` — `actors`
- `suvisdev/apps/viewer/app/dtos/group_model.py` — `groups`
- `suvisdev/apps/viewer/app/dtos/admin_model.py` — `admins`
- `suvisdev/apps/viewer/app/dtos/user_model.py` — `users` (`group_id` → `groups.id`)
