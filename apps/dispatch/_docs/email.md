...
2. **HTTP Request 노드** — 백엔드로 전송
   ...
   - Specify Body: `Using Fields Below` (표현식 모드 `fx` 필요)
   - 필드 매핑:
...
### 실제 구축된 엔드포인트
- **URL**: `http://backend:8000/api/v1/dispatch/inbox`
- **Method**: `POST`
- **Body**: `sender`, `subject`, `body` 3개 필드
...
```
Schedule Trigger (매일 1회) → HTTP Request (watch 재호출, 7단계와 동일 설정)
```
...
- **JSON Body 유효성 오류**: `Using JSON` 모드에서 snippet 등 특수문자 포함 필드는 JSON을 깨뜨림 → `Using Fields Below` + `fx` 표현식 모드 사용
- **`=` 저장 오류**: 필드 값에 `={{ }}` 형식 사용 시 JSON이 무효화됨 → `{{ }}` 만 사용
...
- [x] n8n 셀프호스팅 실행 (Docker Compose)
- [x] Gmail Trigger (폴링, 1분 간격) → HTTP Request 체인 구성
- [x] n8n HTTP Request Body 필드 매핑 (`sender` / `subject` / `body`, `Using Fields Below` + `fx`)
- [x] `POST /api/v1/dispatch/inbox` 엔드포인트 구현 (`InboxReceiveSchema` 3개 필드)
- [x] `dispatch_inbox` 테이블 생성 및 DB 저장 (`InboxRepository.save`)
- [x] `GET /api/v1/dispatch/inbox` 목록 조회 구현 (최신순, 최대 200건)
- [x] `DELETE /api/v1/dispatch/inbox/{id}` 삭제 구현
- [x] 실제 메일 수신 → DB 저장 종단 테스트 완료

### 선택 (Push 방식 전환 시)

- [ ] Cloudflare Tunnel 공개 주소 확보
- [ ] Pub/Sub 토픽 생성 + Gmail Publisher 권한 부여
- [ ] Push 구독 생성 (엔드포인트 = 터널 URL)
- [ ] `users.watch()` 등록 (historyId/expiration 응답 확인)
- [ ] watch 갱신 Schedule Trigger 등록 (매일 1회)
- [ ] 터널 지속성 방안 결정 (Named Tunnel + 도메인)
