...
async def save_actor(
    ...
    return (await actors.save_actor(req)).to_schema()
...
async def list_actors(
    ...
    rows = await actors.list_actors(limit=limit)
...
async def save_actor_name(
    ...
    return (await actors.save_name(req)).to_schema()
...
async def list_actor_names(
    ...
    rows = await actors.list_names(limit=limit)
    return [row.to_schema() for row in rows]
...
async def get_actor(
    ...
    return (await actors.get_actor(actor_id)).to_schema()
